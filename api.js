...
// Free tier: 10 calls/min, covers World Cup (competition code: WC)
// Sign up at https://www.football-data.org/client/register
// Then paste your key in .env as VITE_FD_API_KEY=your_key_here
...
// Known H2H data we've researched — football-data.org free tier
// doesn't include H2H, so we store known matchups here and supplement
// with API data for ongoing tournament matches
const H2H_DATA = {
  'England_Croatia': [
...
function toScorers(match) {
  // football-data.org free tier doesn't give scorers — would need premium
  // Return empty so we gracefully omit the line
  return null
}
...
function formatKickoff(utcDate) {
  ...
  return {
    date_cet:    `${day} ${date} ${mon} ${yr}`,
    kickoff_cet: `${hh}:${mm} CET`,
  }
}
...
// Get yesterday + today matches from the WC
export async function fetchMatches() {
  if (!KEY) throw new Error('NO_API_KEY')

  // Fetch all current WC matches (matchday 1-3, group stage)
  ...
  // Filter to yesterday + today in local time
  const now       = new Date()
